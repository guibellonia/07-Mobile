...
export default function ProdutoFormScreen({ navigation, route }) {
  ...
  // Se estiver editando, carrega os dados atuais do produto
  useEffect(() => {
    if (!editando) return
  ...
  const validarCampos = () => {
    ...
    if (isNaN(qtdNum) || qtdNum < 0) {
      abrirModal('erro', 'Digite uma quantidade válida (número não negativo).')
      return false
    }
    if (!valor.trim()) {
    ...
    if (isNaN(valorNum) || valorNum < 0) {
      abrirModal('erro', 'Digite um valor válido (número não negativo).')
      return false
    }
    return true
  }
